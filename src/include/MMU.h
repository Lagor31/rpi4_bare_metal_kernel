...
#define TD_TABLE (1 << 1)
/*
 * we can check ID_AA64MMFR1_EL1.HAFDBS to see if the hardware supports hw
 * management of the access flag and dirty state, but we'll just set the access
 * flag to 1 by sw.
 */
...
/*
 * memory region shared by all cores, but this has no effect now because
 * non-cachable and device memory are always outer sharable
 * */
#define TD_OUTER_SHARABLE (3 << 8)

#define TD_KERNEL_TABLE_FLAGS (TD_TABLE | TD_VALID)
#define TD_KERNEL_BLOCK_FLAGS                        \
  (TD_ACCESS | TD_OUTER_SHARABLE | TD_KERNEL_PERMS | \
...
#define TD_USER_PAGE_FLAGS                         \
  (TD_ACCESS | TD_OUTER_SHARABLE | TD_USER_PERMS | \
   (MATTR_NORMAL_NC_INDEX << 2) | TD_PAGE | TD_VALID)
...
#define HIGH_MAP_FIRST_END (0x3F400000 + LINEAR_MAP_BASE)
...
/*
  Descriptor format
`+------------------------------------------------------------------------------------------+
 | Upper attributes | Address (bits 47:12) | Lower attributes | Block/table bit
| Valid bit |
 +------------------------------------------------------------------------------------------+
 63                 47                     11                 2 1           0
