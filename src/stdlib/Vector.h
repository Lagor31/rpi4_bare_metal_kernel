...
inline Vector<T>::Vector(const Vector<T>& v) {
  ...
  for (unsigned int i = 0; i < my_size; i++) buffer[i] = v.buffer[i];
}
...
inline Vector<T>::Vector(unsigned int size, const T& initial) {
  ...
  my_capacity = size;
  buffer = new T[size];
  for (unsigned int i = 0; i < size; i++) buffer[i] = initial;
  // T();
}
...
inline void Vector<T>::reserve(unsigned int capacity) {
  ...
  for (unsigned int i = 0; i < l_Size; i++) Newbuffer[i] = buffer[i];
